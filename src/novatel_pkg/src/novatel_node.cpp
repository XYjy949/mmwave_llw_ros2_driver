...
private:
    ...
    rclcpp::Subscription<novatel_oem7_msgs::msg::INSPVA>::SharedPtr subscription_;                   //车辆的姿态信息
...
private:
  ...
  //订阅imu的姿态信息
  void topic_callback(const novatel_oem7_msgs::msg::INSPVA::SharedPtr p_novatelAtt) {
    ...
    //获取车速和yawtae
    vel  = sqrtf ((p_novatelAtt->east_velocity *  p_novatelAtt->east_velocity) + (p_novatelAtt->north_velocity * p_novatelAtt->north_velocity));
    ...
    yawRate =-((deltaAzimuth)/180.f*M_PI)  /  deltaT;   //rad/s
    ...
    // RCLCPP_INFO(get_logger(),"%.2f %.2f", vel, yawRate);
  }
};
